...
function loadCSS(prefix, themeName, replace, callback) {
  var link = document.createElement('link');
  link.rel = 'stylesheet';
  link.href = prefix + 'static/theme_' + themeName + '.css';
  if (callback) {
...
window.addEventListener('load', function() {
  ...
  ajax.onreadystatechange = function() {
    if (ajax.status == 200 && ajax.readyState == XMLHttpRequest.DONE) {
      ...
      var link = loadCSS(prefix, loadDefault('theme'));
      var customCommands = [    
        [/s(witch)? (.*)/, function(m) {
        ...
        [/t(heme)? (.*)/, function(m) {
          var chat = this;
          link = loadCSS(prefix, m[2], link, function(){
            chat.scroll();
          });
        }]
      ];
      
      saveDefault("prefix", prefix);
